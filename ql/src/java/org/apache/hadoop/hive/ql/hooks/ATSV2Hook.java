 ...
 */
public class ATSV2Hook implements ExecuteWithHookContext {

  private static final Logger LOG = LoggerFactory.getLogger(ATSV2Hook.class.getName());
  private static final Object LOCK = new Object();
  private static final int VERSION = 2;
  private static ExecutorService executor;
  private static ExecutorService senderExecutor;
  private static TimelineV2Client timelineClient;
  ...
  private static void setupAtsExecutor(HiveConf conf,TezAMRMClientAsync amRmClient) {
    synchronized(LOCK) {
      if (executor == null) {
        ...
        int queueCapacity = conf.getIntVar(ConfVars.ATSHOOKQUEUECAPACITY);
...
//            final String domainId = createOrGetDomain(hookContext);
      executor.submit(new Runnable() {
        ...
        public void run() {
          try {
            ...
            LOG.info("atsv2hook gethooktype" + hookContext.getHookType());
  ...
  TimelineEntity createPreHookEvent(String queryId, String query, JSONObject explainPlan,
    ...
    atsEntity.setId(queryId);
    atsEntity.setType(EntityTypes.HIVE_QUERY_ID.name());
...
//        }
    ...
    startEvt.setId(EventTypes.QUERY_SUBMITTED.name());
    startEvt.setTimestamp(startTime);
    atsEntity.addEvent(startEvt);

    atsEntity.addInfo(OtherInfoTypes.QUERY.name(), queryObj.toString());
    atsEntity.addInfo(OtherInfoTypes.MAPRED.name(), numMrJobs > 0);
    atsEntity.addInfo(OtherInfoTypes.SESSION_ID.name(), sessionID);
    atsEntity.addInfo(OtherInfoTypes.TEZ.name(), numTezJobs > 0);
    atsEntity.addInfo(OtherInfoTypes.INVOKER_INFO.name(), logID);
    atsEntity.addInfo(OtherInfoTypes.THREAD_NAME.name(), threadId);
    atsEntity.addInfo(OtherInfoTypes.VERSION.name(), VERSION);
    if (clientIpAddress != null) {
      atsEntity.addInfo(OtherInfoTypes.CLIENT_IP_ADDRESS.name(), clientIpAddress);
    }
    atsEntity.addInfo(OtherInfoTypes.HIVE_ADDRESS.name(), hiveInstanceAddress);
    atsEntity.addInfo(OtherInfoTypes.HIVE_INSTANCE_TYPE.name(), hiveInstanceType);
    atsEntity.addInfo(OtherInfoTypes.CONF.name(), confObj.toString());
    if (llapAppId != null) {
      atsEntity.addInfo(OtherInfoTypes.LLAP_APP_ID.name(), llapAppId.toString());
    }
    //atsEntity.setId(domainId);
...
//        }
    ...
    stopEvt.setId(EventTypes.QUERY_COMPLETED.name());
    stopEvt.setTimestamp(stopTime);
    atsEntity.addEvent(stopEvt);

    atsEntity.addInfo(OtherInfoTypes.STATUS.name(), success);
    LOG.info("atsv2hook atsentity" + atsEntity);
    // Perf times
    JSONObject perfObj = new JSONObject(new LinkedHashMap<>());
    for (Map.Entry<String, Long> entry : durations.entrySet()) {
      perfObj.put(entry.getKey(), entry.getValue());
    }
    atsEntity.addInfo(OtherInfoTypes.PERF.name(), perfObj.toString());
    //atsEntity.setId(domainId);
  ...
  void fireAndForget(final TimelineEntity entity) throws Exception {
    senderExecutor.submit(new Runnable() {
      ...
      public void run() {
        try {
          LOG.info("atsv2hoo fireandforget" + entity);
  ...
  private ApplicationId determineLlapId(final HiveConf conf, QueryPlan plan) throws IOException {
    ...
    for (TezTask tezTask : Utilities.getTezTasks(plan.getRootTasks())) {
      ...
      String hosts = HiveConf.getVar(conf, ConfVars.LLAP_DAEMON_SERVICE_HOSTS);
